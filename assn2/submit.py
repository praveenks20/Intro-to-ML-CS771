...
class Node:
	...
	def process_node(self, all_words, my_words_idx, history, verbose):
		...
		if len( history ) == 0:
			...
			# Perform the split for the chosen query index
			split_dict = {}
		...
		# Get a list of all query indices not present in the history
		history_query_idxs = [t[0] for t in history]
		# available_query_idxs = [i for i in range(len(all_words)) if i not in history_query_idxs]
		available_query_idxs = [i for i in my_words_idx if i not in history_query_idxs]
		

		# Compute entropy for each available query index
		entropies = []
		for query_idx in available_query_idxs:
			...
			if len(split_dict) < 2:
				...
				p = [len(s) / len(my_words_idx) for s in split_dict.values()]
				entropy += -np.sum(p * np.log2(p))

			entropies.append(entropy)
		
		# Find the query index with the maximum entropy
		query_idx = available_query_idxs[np.argmax(entropies)]
		query = all_words[query_idx]
